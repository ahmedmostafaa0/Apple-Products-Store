...
const IPhone = () => {
  return (
    <div className="w-full h-full bg-white flex flex-col items-center justify-around p-8">
      ...
      <div className="flex justify-between w-full">
        {iphoneModels.map((iphone, index) => (
            <div key={index} className={`flex flex-col items-center justify-between gap-y-2 ${index === 2 && 'hidden md:flex'}`}>
                <img src={iphone.img} alt={iphone.name} className='2xl:w-72 lg:w-64 sm:w-48 md:h-64 sm:h-48 object-contain' draggable='false' />
                ...
                <button type='button' className='bg-blue-400 text-sm mt-4 rounded-full px-4 py-2 text-white cursor-pointer'>Buy</button>
            </div>
        ))}
      </div>
    </div>
