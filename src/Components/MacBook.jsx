...
const MacBook = () => {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-around p-8 bg-white">
      ...
      <div className="flex">
        {macBookModels.map((mb, index) => (
          <button onClick={() => handleSelectedChip(index)} key={index} 
              ...
              `}
              >
            <span className="2xl:text-lg xl:text-sm text-xs font-bold text-gray-800">With {mb.chip}</span>
      ...
      <div className="flex flex-col 2xl:p-4 md:items-start items-center">
        <div className="w-80 h-64 2xl:mb-0 -mb-8">
          <img src={currentColor.img} alt={`MacBook Air with ${currentMb.chip}`} className="2xl:w-72 xl:w-52
          w-48 h-full 2xl:mt-0 sm:-mt-6 object-contain m-auto" />
        </div>
          ...
          <div className="flex space-x-2 mb-4 md:text-left justify-center md:justify-start">
            {currentMb.colors.map((col, index) => (
              <button key={index} style={{backgroundColor: col.bg}} onClick={() => handleSelectedColor(index)} className={`2xl:w-6 sm:w-5 w-4 2xl:h-6 sm:h-5 h-4 rounded-full border hover:border-blue-500 ${selectedColor === index ? 'border-2 border-blue-400' : 'border-transparent'}`}></button>
            ))}
          </div>
          <img src={currentMb.icon} alt={`Apple ${currentMb.chip} Icon`} className="w-12 h-12 2xl:mb-4 xl:mb-4" />
          ...
          <button type="button" className="w-min bg-blue-400 text-sm py-1 px-3 rounded-full text-white mt-4 md:mt-0">Buy</button>
      </div>
    </div>
  );
};
